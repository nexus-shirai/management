...
const goToEdit = (category_id) => {
    location.href = route('edit-category', { 'category_id': category_id });
};
const onClickDelete = (category_id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.delete(route('delete-category', { 'category_id': category_id }), {
                onFinish: () => {
                    categoryTableRef.value.rerender();
                },
            });
        }
    });
    ...
    <main class="container flex-1 py-5 mx-auto max-w-[1000px]">
        <div class="bg-slate-100 py-2 px-3 my-2">
            ...
            <GridTable
                ...
                :data_url="route('search-categories')"
...
<style>
#category_grid_table th:last-child,
#category_grid_table td:last-child {
